...
export function CallDetailsNavigation({
    ...
    // Comprobamos si la ruta actual es exactamente la de submissions de esta convocatoria
    const isSubmissions = pathname.endsWith(`/calls/${callId}/submissions`)

    return (
        <div className="flex border-b border-slate-100 gap-6 mt-4">
            <Link 
                href={`/admin/calls/${callId}`} 
                className={cn(
            ...
            <Link 
                href={`/admin/calls/${callId}/submissions`} 
                className={cn(
                    "flex items-center gap-2 pb-3 text-sm font-semibold transition-all relative px-1",
                    isSubmissions 
                        ? "text-blue-600 font-bold" 
