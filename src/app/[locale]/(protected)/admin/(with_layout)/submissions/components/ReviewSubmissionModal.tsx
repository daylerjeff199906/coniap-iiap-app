            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <div className="flex justify-between items-start">
                        <div>
                            <DialogTitle className="text-xl font-bold leading-tight">{submission.title}</DialogTitle>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                    <div className="md:col-span-2 space-y-4">
                        <section className="bg-muted/30 p-4 rounded-lg border">
                            ...
                            <div className="flex items-center gap-3">
                                <Avatar className="h-10 w-10"><AvatarFallback><User /></AvatarFallback></Avatar>
                                <div>
                                    <p className="font-medium">{submission.profile?.first_name} {submission.profile?.last_name}</p>
                                    <p className="text-xs text-muted-foreground flex items-center gap-1">
                                        <Mail className="h-3 w-3" /> {submission.profile?.email}
                                    </p>
                                </div>
                            </div>
                        </section>
                                ...
                                <div className="grid gap-2">
                                    {submission.files.map((file) => (
                                        <div key={file.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                                            <div className="flex items-center gap-3">
                                                <div className="p-2 bg-primary/10 rounded"><FileText className="h-5 w-5 text-primary" /></div>
                                                <div><p className="text-sm font-medium">{file.file_name}</p><p className="text-xs text-muted-foreground">{file.document_type}</p></div>
                                            </div>
                                            <a href={file.file_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-sm font-medium text-primary hover:underline">
                                                <Download className="h-4 w-4" /> Descargar
                                            </a>
                                        </div>
                                    ))}
                                </div>
                            ) : (<p className="text-sm text-muted-foreground border border-dashed rounded-lg p-4 text-center">No hay archivos.</p>)}
                        </section>
                    </div>
                    ...
                    <div className="md:col-span-1 space-y-4">
                        <section className="bg-primary/5 p-4 rounded-lg border border-primary/10">
                            ...
                            <select id="status" value={submission.status} onChange={handleStatusUpdate} disabled={isUpdatingStatus} className="flex h-10 w-full rounded-md border bg-background px-3 py-2 text-sm">
                        ...
                        <section className="border rounded-lg flex flex-col h-[320px]">
                            <div className="p-3 border-b bg-muted/50 rounded-t-lg flex items-center gap-2"><MessageSquare className="h-4 w-4 text-primary" /><h3 className="font-semibold text-sm">Feedback</h3></div>
                            ...
                            <form onSubmit={handleAddComment} className="p-3 border-t flex gap-2">
                                <Input placeholder="Escribe un comentario..." className="text-xs h-9 flex-1" value={newComment} onChange={(e) => setNewComment(e.target.value)} />
